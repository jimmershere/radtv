#!/usr/bin/env bash
# R&Dtv one-shot installer for Linux + macOS.
#
...
#      R&Dtv playlist + EPG URLs.
#   5. Copy the R&Dtv repository zip to Kodi's addons/packages cache so the
...
#   bash install.sh --accept-disclaimer  # skip the prompt (also: RADTV_ACCEPT_DISCLAIMER=1)
...
DRY_RUN=0
ACCEPT_DISCLAIMER="${RADTV_ACCEPT_DISCLAIMER:-0}"
...
# the "" defaults in radtv.conf.example.
...
================================================================================
 R&Dtv -- legal and privacy notice (read DISCLAIMER.md for the full text)
================================================================================

 - R&Dtv is GPL-3.0 packaging software. NO WARRANTY. Use at your own risk.
 - R&Dtv does NOT host, transmit, or mirror any audiovisual content. It
 ...
 - R&Dtv is NOT affiliated with NBCUniversal ("The Black Donnellys"), the
...
require_disclaimer() {
  ...
  show_disclaimer
  if [[ ! -t 0 ]]; then
    warn "stdin is not a TTY -- re-run with --accept-disclaimer or RADTV_ACCEPT_DISCLAIMER=1"
    exit 1
  fi
  local reply=""
...
M3U_URL="${IPTV_M3U_URL_OVERRIDE:-$RADTV_REPO_RAW_URL/iptv/dist/radtv.m3u}"
EPG_URL="${IPTV_EPG_URL_OVERRIDE:-$RADTV_REPO_RAW_URL/iptv/dist/radtv.xml}"
...
case "$SKIN_TARGET" in
  ...
  none)                   SKIN_ADDON="" ;;
  *) warn "Unknown RADTV_SKIN_TARGET=$SKIN_TARGET"; SKIN_ADDON="" ;;
esac
if [[ -n "$SKIN_ADDON" ]]; then
  SKIN_COLORS_DIR="$ADDONS_ROOT/$SKIN_ADDON/colors"
  SKIN_SRC="$REPO_ROOT/build/wizard/resources/skin/$SKIN_TARGET/colors/radtv.xml"
  if [[ -d "$SKIN_COLORS_DIR" ]]; then
    note "Copying R&Dtv color override into $SKIN_COLORS_DIR"
    run "cp \"$SKIN_SRC\" \"$SKIN_COLORS_DIR/radtv.xml\""
    ok "R&Dtv theme staged for $SKIN_ADDON (select via Settings > Skin > Colours > radtv)"
  else
...
echo "  2. Install repository.radtv via 'Install from zip file'."
echo "  3. Launch R&Dtv Wizard from Programs to finish auth + install scrapers."
